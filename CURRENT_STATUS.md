...
**Session (2026-06-22b) — Stage 3 Phase 5.1 + Phase 3 COMPLETE & live-verified. Branch `master`. Phase 5.1 PUSHED (`f0bf3bd..96328a4`); Phase 3 (`57948fc`+`3f22503`) local.** Plan: `docs/plans/2026-06-14-stage3-ai-motion.md`. Tests: **3166 pytest + 514 vitest, tsc clean.**

- **Phase 3 — DART networked service, LIVE round-trip PROVEN** (`57948fc` contract + `3f22503` proof). Filled the motion-server AI stub: box `/generate` → resident `DartRunner` → clip artifact `{kind:"clip",format:"npz",npz_b64,…}`; Mac `forward_generate` decodes → `dart_to_glb` → `/files/animations/dart-generated/<stem>_<hash>.glb` → `{kind:"clip",format:"glb",url,…}`; `api.ts` union mirror + `viewerStore.dispatchClip`/`dispatchMotionResponse`. **Deployed `backend.motion` to the box, ran `motion_server` in the WSL `dart` env (DART loaded, `/status` `dart:true`), and round-tripped from the Mac: wave (cached 234ms) + cheer (fresh 818ms) — both render-gated upright/grounded/animating** (`docs/testing/screenshots/2026-06-22-stage3-phase3/`). Box setup (FastAPI deps, scp+extract, /root/DART launch, WSL2 portproxy, foreground-ssh keepalive) + remaining daemon-hardening are in the plan status log. **Box service is NOT yet a durable daemon** (dies when the launching ssh closes; portproxy persists but WSL IP is dynamic).
...
- **NEXT:** (1) productionize the box DART service as a durable daemon (Windows scheduled task/service launching WSL; re-point portproxy to the dynamic WSL IP on boot); (2) Phase 5.x — wire Kokoro turns to *request* live-generated clips via `motion_remote_url` (vs the pre-baked Phase-5.1 library); (3) Phase 6 EMAGE (CC-BY-NC-SA license decision). Commercial SMPL/data licensing applies before ship.
