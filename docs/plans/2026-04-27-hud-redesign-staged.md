...
- 2026-04-27 Tier 1: ✓ 5 deletes/moves shipped — ContextBudgetPill moved into StatusBar (kills overlap with starters), `Next:` inline line replaced with native title-tooltip on bond pill (header drops 1 row), TemperatureMeter dead UI removed, duplicate viewer Close button deleted (Back-to-Chat in top bar stays), `Ctrl+0` viewer hint corrected to `Dbl-click` and iframe bumped `?v=8`→`?v=9`. Browser-verified via Playwright: pill renders at top-right, bond bar tighter, no overlap, hint label correct in served HTML. pytest 2683 ✓ vitest 203 ✓ tsc clean. Pause + evaluate before Tier 2.
- 2026-04-29 Tier 5: ✓ sidebar bottom toolbar 6 → 4 cells. Memory · Lore stay always-visible; Games · Stats · Context collapsed into MoreToolsDropdown (`⋯ More`, mirrors HelpDropdown pattern, opens upward). Help (`?`) and NotificationBadge unchanged. Both expanded (4-col grid) and collapsed (vertical icon column) layouts updated. Browser-verified: light theme (sakura-dawn) + dark theme (tokyo-night) — popover renders, items clickable. Known parity issue: popover bg uses `--color-surface` like HelpDropdown; in some dark themes the surface token matches sidebar bg so the popover blends in (pre-existing pattern, not a Tier 5 regression). pytest 2684 ✓ vitest 207 ✓ tsc clean. Audit ref: `docs/research/2026-04-27-hud-element-audit.md` Zone 8.
