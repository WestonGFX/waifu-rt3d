...
- 2026-04-29 (session 23): **Tier 1 SHIPPED.** Pure-frontend, no LLM cost. Approach refined mid-impl: existing `tokenizeMarkdown` parser in `DialogueBubble.tsx` already handled `**bold**` / `*italic*` / `(narration)` for assistant messages — factored to `frontends/sakura/src/lib/parseActions.ts` with 18-case Vitest suite, then reused for **both** assistant + user message rendering (user side previously rendered raw via `HighlightedText`). Italic tokens now style with `var(--color-accent)` at 0.85 opacity (assistant) or `var(--color-accent-text)` at 0.85 (user — bubble bg is accent gradient, accent-on-accent would be invisible). Composer toolbar gained Lucide `Italic` button + textarea-local Ctrl/Cmd+I shortcut (composer-local handler avoids collision with global `ctrl+i = Cinematic mode` since global has `allowInInput: false`). Verified: tsc clean · vitest 225 pass (+18) · pytest 2684 pass · light theme (sakura-dawn, white italic on pink gradient) + dark theme (catppuccin-macchiato, dark italic on muted pink) both readable. Screenshots in `docs/testing/screenshots/2026-04-29-tier1-action-syntax/`. Open Questions 1-4 still block Tiers 2/3.
- 2026-05-01 Tier 1 follow-up (RichComposer wire-up): ✓ commit e94384c — live italic in composer, dark/light theme contract verified
- 2026-05-06 (session 25): **Tier 2 SHIPPED.** Reframe — the existing `quickChips` system in `ChatThread.tsx` already produces user-side reply suggestions (the prompt asks for "replies the user might send"); plan's "AI quickChips" framing was a misread. So Tier 2 here = **configure + persona-enhance the existing pills**, not build a parallel component. **Open Questions resolved in plan-mode (locked positions):** Q1 persona — reuse existing `user_persona` config (no new schema); Q2 pill count — default 3, new `reply_pills_count` setting picks 2 or 3; Q3 click — default **AUTO-FILL** (was auto-send), new `reply_pills_action` setting picks `'fill'` or `'send'`; Q4 model — same provider as chat (Tier 3 deferred). **Code:** `_KNOWN_CFG_KEYS` +2 in `backend/server.py`; `AppConfig` +2 typed optional fields in `frontends/sakura/src/lib/types.ts`; `api.generateQuickReplies` gains `opts: { count?: 2|3; userPersona?: string }` with `[About the user]: …` prefix injection; `generateChips` + `generateChipsLLM` take `count`; effect threads `config.reply_pills_count` + `config.user_persona`; chip click branches on `reply_pills_action` (fill → `setDraft` + focus, send → `sendMessage`); Settings → General → Chat Behaviour gains 2 segmented-pill rows (Pill count `2|3`, On click `Fill composer | Send immediately`), gated dim on `showQuickChips`. **Tests:** new `frontends/sakura/src/test/replyPills.tier2.test.ts` — 6 cases stubbing `fetch` against the real `api.generateQuickReplies`: default count=3 + tone rule, count=2 + 2-tone rule + max_tokens=90, persona injection ordering, empty/whitespace persona omission, count+persona combo. **Verification:** vitest 225 → 231 ✓ (6 new), tsc 3 preexisting errors only (`viewer.blinkController.test.ts` node:fs/path/__dirname — unchanged from master baseline). pytest not run in remote (no .venv); backend change is two literals added to a warning-only validator set so cannot regress any test.
