...
- 2026-05-10 M7 Spring Bones (item 24): ✓ ALL 5 phases complete. P1+P3-backend+P4 were pre-existing; P2 (capsule collider debug), P3-frontend (preset save/load + auto-apply), P5 (emotion-reactive) closed in commit 2a6de6d.
- 2026-05-10 M7 Jiggle Physics (item 25): ✓ Phases 1-4 complete (P1+P2 pre-existing; P3 character_physics_profiles v82 + API + panel UI; P4 emotion+movement multipliers wired). Phases 5-6 deferred (advanced, model-specific). commit 740a78c.
- 2026-05-10 M7 Humanoid Motion (item 26): ✓ Phases A-E complete. A (spring math + PoseSpringManager), B (SpringChain + FollowThroughLayer + breathing/weight-shift), C (BoneMaskManager + InertializationBlend + spring LookAt), D (18 gestures + BalanceLayer + chat→gesture wiring), E (PoseLibrary + TwoBoneIK + FootPlantController + manifest endpoint). Phase F (Neural Motion) deferred — GPU server required. commits 89dd869..eaa5483.
