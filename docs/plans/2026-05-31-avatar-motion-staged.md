...
- **2026-06-14 — Stage 3 (AI motion) planned → dedicated execution plan: [`docs/plans/2026-06-14-stage3-ai-motion.md`](2026-06-14-stage3-ai-motion.md).** Supersedes the stale MotionLCM/MoMask picks in the Stage 3 section above (lines 98–104) with DART (Apache-2.0, streaming, direct rotations) per `docs/research/2026-06-08-ai-motion-models.md`. Execution harness = Claude drives the RTX 5080 via SSH-from-this-session (Chrome Remote Desktop bootstrap → SSH primary → Claude-Code-on-box fallback). Reuses the Bug-2 normalized-rotation injection harness. NOT started — Phase 0 (enable SSH) awaits the box being powered on + reachable.
