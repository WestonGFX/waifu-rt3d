...
- **2026-06-22 — Phase 3 design + roadmap drafted (grounded in the real motion-server code).** [`docs/research/2026-06-22-stage3-phase3-design.md`](../research/2026-06-22-stage3-phase3-design.md). Read `motion_server.py`/`remote_client.py`: the live contract speaks **euler keyframes**, but Phase 2 produces a **normalized GLB clip** → Phase 3 reconciles them. Recommendations: response = **clip artifact** (reuse the gate-proven `dispatchLoadAnimation` path, not the procedural keyframe path → avoids re-opening Bug-2); transport = **box returns npz, Mac runs `dart_to_glb.py`** (reuses the tool, keeps SMPL/license machinery on the box); DART as a **persistent service** inside `motion_server.py` (must run in the box's WSL `dart` conda env, not the repo `.venv`) + a `(prompt,duration,seed)` cache. **Strong bridge idea: a pre-baked DART gesture library** — generate ~8 companion gestures once on the box, convert+gate offline, ship as the avatar's gesture vocabulary with ZERO runtime GPU dependency (mirrors the Stage-1 Mixamo clips). Awaiting Chris's decisions (gesture list / Phase 3 contract approval / emotion→motion mapping / EMAGE license).
- **2026-06-22 — Phase 3 ENGINE built + live-verified; networked service layer remains.** `backend/motion/dart_runner.py` = a resident-model DART wrapper (`DartRunner.load()` once → `generate(prompt, primitives, seed) → npz Path`), so the diffusion model stays in VRAM and each clip is ~1.3 s (vs ~14 s cold). DART imports are **lazy** so the file is importable on the Mac (4 Mac-side pytest: imports-without-DART, construction, generate-before-load raises). **Live-verified on the box** (scp'd to `/root/DART`, ran in the `dart` conda env): generated a `nod head` clip via the resident interface; pulled the npz → `dart_to_glb --face-camera` → render gate 22/22 tracks, 6/6 distinct frames → engine→GLB loop closed. **Remaining for the live Mac↔box path (gated on deploying the waifu repo to the box as a persistent service in the `dart` env):** wire `motion_server._try_load_ai_backend`/`/generate` AI branch to call `DartRunner` and return a clip-artifact (`kind:"clip", npz_b64`), extend `remote_client.forward_generate` to decode→`dart_to_glb`→serve a `/files` URL, advertise `dart` in `/status`, mirror the response in `api.ts`, then live round-trip verify. Pre-baked gesture library already delivers GPU-free gesture playback in the meantime. Engine cost confirmed: 2.8 GB VRAM, fits trivially.
- **2026-06-22 — Phase 6 (EMAGE co-speech) SCOPED.** [`docs/research/2026-06-22-emage-cospeech-scoping.md`](../research/2026-06-22-emage-cospeech-scoping.md). Deliberate next-session stand-up (new model family, multi-GB weights, **CC BY-NC-SA licensing gate** to resolve first). Reuses the Phase-2 win — EMAGE outputs SMPL-X so `dart_to_glb.py` applies to body+hands; face → existing ARKit blendshapes. Stand-up plan mirrors the DART Phase-1 playbook (box env, weights, smoke test, GLB seam, audio sync). Recommend sequencing AFTER the Phase 3 networked service + Phase 5.1 emotion→motion. Open decisions for Chris: license intent (prototype vs ship) + priority order.
- **2026-06-22 — Phase 3 ✓ COMPLETE: DART networked service live round-trip verified Mac↔box.** `57948fc` (contract) + docs. The motion-server AI stub is filled end-to-end. **Contract (Decision 1=A clip-artifact, Decision 2=A2 npz→Mac, Decision 3=persistent DART):** box `/generate` AI branch → resident `DartRunner` → `{kind:"clip",format:"npz",npz_b64,name,prompt,fps,primitives,backend:"dart",…}`; `_try_load_ai_backend` loads DART once at startup (lazy import → Mac-importable); `/status` advertises `dart`; emotion→prompt + duration→primitives maps + bounded `(prompt,primitives,seed)` cache. Mac `remote_client.forward_generate` collapses the artifact → decodes npz → `tools/dart_to_glb` (face_camera) → `backend/storage/animations/dart-generated/<stem>_<sha1[:8]>.glb` (content-hash dedup) → `{kind:"clip",format:"glb",url,…}`; `server.py /api/motion/generate` forwards prompt/seed + passes either union arm through; `api.ts` mirrors `MotionGenerateResponse`; `viewerStore.dispatchClip`/`dispatchMotionResponse` play it. +12 pytest +4 vitest (3166/514), tsc clean.
  - **LIVE round-trip PROVEN (2026-06-22):** deployed the `backend.motion` package to the box (`/root/DART/backend/`), ran `motion_server` in the WSL `dart` conda env from `/root/DART` (`python -m backend.motion.motion_server --port 8081`); DART loaded in ~6 s, `/status` → `"dart":true`. From the Mac, `forward_generate('http://10.0.0.2:8081', {emotion:happy})` → **wave** clip (cached, 234 ms) and `{prompt:'raise both hands',seed:7}` → **cheer** clip (fresh generation, 818 ms) — both decoded→converted→**render-gated upright/grounded/22-track/6-of-6-distinct-frames** (`docs/testing/screenshots/2026-06-22-stage3-phase3/roundtrip-{wave,cheer}.png`). The full chain emotion→box-DART→npz→Mac-GLB→viewer is live-verified.
  - **Reproducible box setup (deployment knowledge):** (a) `dart` env needed `pip install fastapi "uvicorn[standard]" pydantic` (was inference-only); (b) `scp` a tar of `backend/{__init__,motion/*}.py` → `C:/dev/`, extract at `/root/DART/` (WSL reads `/mnt/c/dev/`); (c) launch from `/root/DART` so `mld` (cwd) + `backend.*` both import; (d) **WSL2 NAT blocks LAN access** → added a Windows portproxy `netsh interface portproxy add v4tov4 listenport=8081 connectaddress=<wsl-ip>` + firewall rule (persists; `<wsl-ip>` = `wsl hostname -I`, changes on reboot); (e) the server must stay foreground inside a held-open ssh OR be daemonized — **nohup alone dies when the WSL VM tears down** after the launching ssh returns.
  - **Remaining hardening (not blocking):** productionize the box service as a real daemon (Windows scheduled task / service launching WSL) so it survives reboot + needs no held-open ssh, and re-point the portproxy to the (dynamic) WSL IP on boot. Frontend currently reaches DART motion only via `motion_remote_url`; wiring Kokoro turns to *request* generated clips (vs the pre-baked Phase-5.1 library) is a Phase-5.x follow-up. Commercial SMPL/data licensing still applies before ship.
- **2026-06-22 — Phase 5.1 ✓ emotion→motion wired (pre-baked gesture path).** `2361c07`. New `frontends/sakura/src/lib/dartGestures.ts`: maps Kokoro's per-turn `emotion` → a pre-baked DART gesture (`KOKORO_EMOTION_TO_DART_GESTURE`: excited/proud→cheer, frustrated→cross_arms, sleepy→stretch, playful→shrug — sparse on purpose; common/gentle emotions stay still), plus `dartGestureUrl`/`resolveDartGesture` + a tunable `DART_GESTURE_COOLDOWN_TURNS=3`. `viewerStore` gains `dispatchDartGesture(name)` — load-then-play the normalized-VRM GLB through the gate-proven `loadAnimation`/`playAnimation` path (`dart_`-prefixed clip names, VRM-only, cleared on model load like the Mixamo set). **Firing policy (Chris chose 2026-06-22): gap-fill + throttled** — `dispatchKokoroEmbodiment` Step 4 fires an emotion gesture ONLY when the LLM picked no explicit gesture, the emotion maps, and the cooldown elapsed; explicit Kokoro gestures (wave/point/…) still use the proven Mixamo clips untouched. **No viewer.html change** (sensitive area). Degrades gracefully — a missing DART GLB just no-ops (same as the Mixamo missing-clip path), so this is inert-but-correct on the Mac until the gesture GLBs are built on the box (`tools/build_dart_gestures.py`) and copied to `backend/storage/animations/dart-gestures/`. +12 vitest (510 total), tsc clean. **Visual verification deferred** to once the GLBs exist locally. **NEXT (still gated): Phase 3 networked service** (box-deploy) — Phase 5.1's clip-load seam is now also the runtime sink that path will feed.
