...
/**
 * Glass-backed chat input bar with slash command support.
 *
 ...
 * a subtle glow on focus, and an autocomplete dropdown for `/commands`.
 * Springs in from below on mount.
 *
 * When the input starts with `/`, an autocomplete dropdown appears above
 * the input showing matching commands. Arrow keys navigate, Enter selects
 * or submits, and Escape dismisses the dropdown.
 *
 * The input expands vertically for multi-line messages (up to 4 lines)
 * using a textarea that auto-resizes.
 *
 * @param onSend - Called with the message text when user sends a normal message
...
export function InputBar({
  ...
  const [matchingCommands, setMatchingCommands] = useState<SlashCommand[]>([]);
  const [selectedIndex, setSelectedIndex] = useState(0);
  const [showAutocomplete, setShowAutocomplete] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);

  /**
   * Recompute matching commands whenever the input text changes.
   * Only shows the dropdown when the input starts with `/`.
   */
  useEffect(() => {
    if (text.startsWith('/')) {
  ...
  /**
   * Scroll the active autocomplete item into view when the selection changes.
   */
  useEffect(() => {
  ...
  /**
   * Fill the input with a selected command name and a trailing space
   * so the user can immediately start typing arguments.
   */
  ...
  const handleSend = useCallback(async () => {
    ...
    // Route slash commands through the command parser
    if (trimmed.startsWith('/')) {
      const context = buildCommandContext();
      const handled = await parseAndExecute(trimmed, context);
      if (handled) {
        setText('');
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (showAutocomplete && matchingCommands.length > 0) {
      ...
      if (e.key === 'Enter' && !e.shiftKey) {
        e.preventDefault();
        // If the user hasn't typed a full command yet (no space after /),
        // selecting from autocomplete fills the input instead of executing
        const afterSlash = text.slice(1);
        const hasSpace = afterSlash.includes(' ');
        if (!hasSpace && matchingCommands.length > 1) {
          selectCommand(matchingCommands[selectedIndex]);
        } else {
          handleSend();
        }
  ...
  }, [handleSend, showAutocomplete, matchingCommands, selectedIndex, selectCommand, text]);
          ...
          >
            {matchingCommands.map((cmd, i) => (
              <div
                ...
                onMouseDown={(e) => {
                  // Prevent textarea blur before we fill the command
                  e.preventDefault();
                  selectCommand(cmd);
                }}
              >
                ...
                {cmd.argPlaceholder && (
                  <span className={styles.commandArgs}>{cmd.argPlaceholder}</span>
                )}
